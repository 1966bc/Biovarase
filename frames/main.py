...
class Biovarase(ttk.Frame):
    ...
    def on_update_result(self, event=None):
        ...
        if self.lstResults.curselection():
            ...
            if self.enable_rejections.get() == False:
                obj = frames.result.UI(self, engine=self.engine, index=index)
            else:
                obj = frames.rejections.UI(self, engine=self.engine)
    ...
    def set_lj(self, count_rs, target, sd, series, count_series,
        ...
        for x, y in enumerate(series):
            ...
            if show_error_bar:
                self.lj.errorbar(x, y, yerr=self.engine.percentage(et, y),
    ...
    def on_analytical_goals(self):
        frames.analytical_goals.UI(self, engine=self.engine).on_open()

    def on_tests(self,):
        frames.tests.UI(self, engine=self.engine).on_open()

    def on_units(self,):
        frames.units.UI(self, engine=self.engine).on_open()

    def on_elements(self,):
        frames.elements.UI(self, engine=self.engine).on_open()

    def on_set_zscore(self,):
        frames.set_zscore.UI(self, engine=self.engine).on_open()

    def on_zscore(self,):
        frames.zscore.UI(self, self.engine).on_open()

    def on_data(self,):
        frames.data.UI(self, engine=self.engine).on_open()

    def on_actions(self,):
        frames.actions.UI(self, engine=self.engine).on_open()

    def on_analitical(self,):
        frames.analytical.UI(self, engine=self.engine).on_open()

    def on_quick_data_analysis(self,):
        frames.quick_data_analysis.UI(self, engine=self.engine).on_open()

    def on_export_rejections(self,):
        frames.export_rejections.UI(self, engine=self.engine).on_open()

    def on_export_counts(self,):
        frames.counts.UI(self, self.engine).on_open()
    ...
    def on_plots(self,):
        ...
        if self.cbTests.current() != -1:
            ...
            frames.plots.UI(self, engine=self.engine).on_open(selected_test)
        else:
    ...
    def on_tea(self,):
        ...
        if self.cbTests.current() != -1:
            ...
            pk = self.dict_tests[index]
            selected_test = self.engine.get_selected("tests", "test_id", pk)
            frames.tea.UI(self, engine=self.engine).on_open(selected_test)
        else:
            msg = "Not enough data to plot.\nSelect a test."
            messagebox.showwarning(self.engine.title, msg, parent=self)
    ...
    def on_add_batch(self):
        ...
        if self.cbTests.current() != -1:
            obj = frames.batch.UI(self, engine=self.engine, index=None)
    ...
    def on_update_batch(self, evt=None):
        ...
        if self.lstBatches.curselection():
            index = self.lstBatches.curselection()[0]
            obj = frames.batch.UI(self, engine=self.engine, index=index)
    ...
    def on_add_result(self,):
        ...
        if self.lstBatches.curselection():
            obj = frames.result.UI(self, engine=self.engine, index=None)
            obj.on_open(self.selected_test, self.selected_batch)
        else:
