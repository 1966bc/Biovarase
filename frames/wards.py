...
class UI(tk.Toplevel):
    ...
    def init_ui(self):
        ...
        self.lblSections = tk.LabelFrame(frm_right, text="Wards")
        cols = (["#0", "id", "w", False, 0, 0],
                ["#1", "Manager", 'w', True, 200, 200],
                ["#2", "Ward", 'w', True, 200, 200],)

        self.lstWards = self.nametowidget(".").engine.get_tree(self.lblSections, cols)
        self.lstWards.tag_configure('status', background=self.nametowidget(".").engine.get_rgb(211, 211, 211))
        self.lstWards.bind("<<TreeviewSelect>>", self.on_ward_selected)
        self.lstWards.bind("<Double-1>", self.on_ward_activated)
    ...
    def set_values(self):
        ...
        for i in self.lstWards.get_children():
            self.lstWards.delete(i)
    ...
    def set_wards(self, args):
        ...
        sql = "SELECT wards.ward_id,\
                      users.last_name ||' '|| users.first_name,\
                      wards.ward,\
                      wards.status\
               FROM wards\
               INNER JOIN users ON wards.user_id = users.user_id\
               WHERE wards.site_id = ?;"
        ...
        if rs:
            ...
            for i in rs:
                ...
                self.lstWards.insert('', tk.END, iid=i[0], text=i[0],
                                            values=(i[1], i[2], i[3]),
                                            tags=tag_config)
                
        s = "{0} {1}".format("Wards", len(self.lstWards.get_children()))                
        self.lblSections["text"] = s       
        
    def on_ward_selected(self, evt=None):

        if self.lstWards.focus():
            item_iid = self.lstWards.selection()
            pk = int(item_iid[0])
            self.selected_ward = self.nametowidget(".").engine.get_selected( self.table, self.primary_key,  pk)
    ...
    def on_ward_activated(self, evt=None):
        ...
        if self.lstWards.focus():
            item_iid = self.lstWards.selection()
            pk = int(item_iid[0])
            selected_ward = self.nametowidget(".").engine.get_selected( self.table, self.primary_key, pk)
